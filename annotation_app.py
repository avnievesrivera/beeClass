...
def build_labelbee_url(video: str, frame, video_data_col: Optional[str] = None) -> str:
    ...
    return f"{base}#video_id=/datasets/gurabo4/mp4/col10/{video}&frame={frame_val}&tracks_tagged_data=/datasets/gurabo4/tracks/col10/{vd}&frame={frame}"
...
.path-row .form-group { flex:1; margin:0; }
    ...
    ui.div(
        ...
        ui.card(
            ...
            ui.div(
                ...
                style="display:flex; gap:8px; margin-top:.5rem;",#style="margin-top:.5rem;"
            ),
        ),
        # wire native HTML buttons → Shiny inputs
        ui.tags.script("""
            ['btn_load','btn_prev','btn_next','btn_snap'].forEach(function(id){
        ...
        # button highlights are rendered server-side via ann_panel output
...
def server(input, output, session):
    ...
    csv_fpath    = reactive.Value(None)   # Path — where to autosave
    ...
    loading_flag = reactive.Value(False)  # suppress autosave while populating widgets
    ...
    def _load():
        ...
        group_ids.set(gids)
        cur_idx.set(0)
        status_msg.set(("ok", f"Loaded {p.name} — {len(df)} rows, {len(gids)} groups. Autosave ON → {p}"))
    ...
    def _ann_btn():
        ...
        # clicking the already-active button deselects it (toggle to "")
    ...
    # ── free-text field beside buttons ───────────────────────────────────────
    ...
    # ── text areas → autosave on every keystroke ──────────────────────────────
    ...
    # ── annotation panel (re-renders on every state change, keeps buttons in sync) ──
    ...
    def img_display():
        ...
        if not fname:
            return ui.p("No filename.", style="color:#484f58; padding:1rem;")
